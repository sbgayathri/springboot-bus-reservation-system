...
const BrowseBuses = () => {
  ...
  const handleBookBus = async () => {
    if (!selectedBus || !currentUser?.id) return;
    
    try {
      setBookingLoading(true);
      setError('');
      
      await userService.bookBus(selectedBus.id, currentUser.id, seatsToBook);
      setSuccess(`Successfully booked ${seatsToBook} seat(s) on bus ${selectedBus.busnum}!`);
      setShowBookModal(false);
      setSelectedBus(null);
      setSeatsToBook(1);
      
      // Refresh buses to show updated available seats
      await fetchBuses();
      ...
      setError('Failed to book bus: ' + (err.response?.data?.message || err.message));
    } finally {
        ...
        <Col>
          <Card>
            <Card.Header className="bg-primary text-white">
              ...
              <Row>
                <Col>
                  <h4>🚌 Browse Available Buses</h4>
                  <p className="text-muted mb-0">Find and book your perfect journey</p>
                </Col>
                <Col xs="auto">
                  <Button 
            ...
            <Card.Body>
              ...
              {success && <Alert variant="success">{success}</Alert>}              {loading ? (
